...
from .serializers import FlightSerializer, UserFlightSerializer, OfferSerializer
...
# Flights List and Create
class FlightListCreateAPIView(generics.ListCreateAPIView):
    queryset = Flight.objects.all().order_by('departure_datetime')
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter('ordering', openapi.IN_QUERY, description="Ordering of results. E.g., 'departure_datetime', '-departure_datetime'", type=openapi.TYPE_STRING),
        ],
        responses={
...
# Flights Detail (Retrieve, Update, Delete)
...
class FlightSearchAPIView(generics.ListAPIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('origin', openapi.IN_QUERY, description="Origin city or airport name", type=openapi.TYPE_STRING),
            openapi.Parameter('destination', openapi.IN_QUERY, description="Destination city or airport name", type=openapi.TYPE_STRING),
            openapi.Parameter('departure_date_from', openapi.IN_QUERY, description="Departure date from (YYYY-MM-DD)", type=openapi.TYPE_STRING),
            openapi.Parameter('departure_date_to', openapi.IN_QUERY, description="Departure date to (YYYY-MM-DD)", type=openapi.TYPE_STRING),
            openapi.Parameter('min_weight', openapi.IN_QUERY, description="Minimum available weight", type=openapi.TYPE_NUMBER),
            openapi.Parameter('max_weight', openapi.IN_QUERY, description="Maximum available weight", type=openapi.TYPE_NUMBER),
            ...
            openapi.Parameter('ordering', openapi.IN_QUERY, description="Ordering of results. E.g., 'price', '-departure_datetime'", type=openapi.TYPE_STRING),
        ],
        responses={
        ...
        try:
            ...
            if departure_date_from:
                ...
                if not parsed_date_from:
                    raise ValidationError({'departure_date_from': 'Invalid date format. It must be in YYYY-MM-DD format.'})
            ...
            if departure_date_to:
                ...
                if not parsed_date_to:
                    raise ValidationError({'departure_date_to': 'Invalid date format. It must be in YYYY-MM-DD format.'})
...
# Flight Offers Detail (Retrieve, Update, Delete)
...
class OfferListCreateAPIView(generics.ListCreateAPIView):
    queryset = Offer.objects.all().order_by('price')  # Added ordering
    serializer_class = OfferSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
...
class UserFlightListCreateAPIView(generics.ListCreateAPIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('page', openapi.IN_QUERY, description="Page number", type=openapi.TYPE_INTEGER),
            openapi.Parameter('page_size', openapi.IN_QUERY, description="Number of items per page", type=openapi.TYPE_INTEGER),
            openapi.Parameter('ordering', openapi.IN_QUERY, description="Ordering of results. E.g., 'publish_datetime', '-publish_datetime'", type=openapi.TYPE_STRING),
        ],
        responses={
...
# UserFlight Detail (Retrieve, Update, Delete)
